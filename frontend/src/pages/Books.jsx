    ...
    <div className="flex flex-col items-center justify-center py-20 text-slate-400">
      ...
      {hasSearch ? (
        <>
          ...
          <button onClick={onClear} className="text-xs text-indigo-600 hover:underline mt-1">Clear search</button>
        </>
      ) : (
        <>
...
export default function Books() {
  ...
  return (
    <div className="space-y-5 page-fade-in">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((book, idx) => (
                  <tr key={book.book_id} className="hover:bg-indigo-50/20 transition-colors group">
                    ...
                    <td className="table-cell"><StatusBadge status={book.availability_status} /></td>
                    <td className="table-cell">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setEditBook(book)} title="Edit book"
      ...
      {/* Add Modal */}
      <Modal isOpen={addOpen} onClose={() => setAddOpen(false)} title="Add New Book">
        <BookForm onSubmit={handleAdd} onCancel={() => setAddOpen(false)} loading={submitting} />
      </Modal>
      ...
      <Modal isOpen={!!editBook} onClose={() => setEditBook(null)} title="Edit Book">
        {editBook && (
          <BookForm
            initialData={{ title: editBook.title, author: editBook.author, category: editBook.category, isbn: editBook.isbn }}
            onSubmit={handleEdit} onCancel={() => setEditBook(null)} loading={submitting}
          />
        )}
      </Modal>
      ...
      <ConfirmModal
        ...
        confirmText="Delete Book" loading={deleting}
      />
    </div>
  )
}
