      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className={`form-input ${errors.borrower_name ? 'border-red-400 focus:ring-red-400' : ''}`}
          />
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`form-input pl-9 ${errors.phone ? 'border-red-400 focus:ring-red-400' : ''}`}
            />
          </div>
...
function EmptyState({ hasSearch, onClear }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-slate-400">
      ...
      {hasSearch ? (
        <>
          <p className="text-sm font-medium text-slate-600 mb-1">No borrowers match your search</p>
        ...
        <>
          <p className="text-sm font-medium text-slate-600 mb-1">No borrowers registered yet</p>
          <p className="text-xs">Add your first borrower to get started.</p>
        </>
      )}
    </div>
  )
...
export default function Borrowers() {
  ...
  const handleEdit = async (formData) => {
    ...
    try {
      ...
      toast.success(`${formData.borrower_name} updated successfully!`)
    ...
    <div className="space-y-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((b, idx) => (
                  <tr key={b.borrower_id} className="hover:bg-slate-50/80 transition-colors">
      ...
      <Modal isOpen={!!editBorrower} onClose={() => setEditBorrower(null)} title="Edit Borrower">
        {editBorrower && (
          <BorrowerForm
            initialData={{
              borrower_name: editBorrower.borrower_name,
              email:         editBorrower.email,
              phone:         editBorrower.phone,
            }}
            onSubmit={handleEdit}
            onCancel={() => setEditBorrower(null)}
            loading={submitting}
          />
        )}
      </Modal>
      ...
      <ConfirmModal
        ...
        message={`Remove "${deleteTarget?.borrower_name}" from the system? This cannot be undone. The action will fail if they have active borrows.`}
        confirmText="Remove Borrower"
        loading={deleting}
      />
    </div>
  )
